...
async function saveItem() {
  ...
  let content, scheduledDate = null, scheduledTime = null, scheduleMemo = null;
  ...
  if (isScheduleMode) {
    ...
    scheduleMemo  = ($id('schedule-memo')?.value || '').trim() || null;
    // メモがあれば表示用本文にも結合
    if (scheduleMemo) content += '\n' + scheduleMemo;
  ...
  S.selectedTags = [];
  const scheduleTitleEl = $id('schedule-title');
  if (scheduleTitleEl) scheduleTitleEl.value = '';
  const scheduleMemoEl  = $id('schedule-memo');
  if (scheduleMemoEl)  scheduleMemoEl.value  = '';
  const scheduleDateEl  = $id('schedule-date');
...
function handleExpand(id, card) {
  ...
  actions.innerHTML = `
    <div class="expand-tags">
      ...
      <button class="expand-save-tags" data-save-tags="${id}" title="タグを保存">✓</button>
    </div>

    <div class="expand-footer">
      <span class="expand-date">${fmtDateTime(item.createdAt)}</span>
      <div class="expand-btns">
        <button class="expand-btn edit-btn" data-edit="${id}" title="編集">
        ...
        ${!item.googleEventId && GCal.isConnected()
          ? `<button class="expand-btn gcal-btn" data-gcal="${id}" title="Googleカレンダーに追加">
              <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          : ''}
        <button class="expand-btn delete-btn" data-del="${id}" title="削除">
          <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  actions.querySelector('[data-edit]')?.addEventListener('click', e => {
    e.stopPropagation();
    const contentEl = card.querySelector('.item-content');
...
function openGCalModal(item) {
  ...
  const defaultColor = GCal.getDefaultColor();
  grid.innerHTML = GCAL_COLORS.map(c =>
    `<label class="color-opt" title="${c.name}">
      <input type="radio" name="gcal-color" value="${c.id}" ${c.id === defaultColor ? 'checked' : ''}>
...
// ─── Settings View ────────────────────────────────────────────────────────────
function renderGCalDefaultColor() {
  const grid = $id('gcal-default-color');
  if (!grid) return;
  const current = GCal.getDefaultColor();
  grid.innerHTML = GCAL_COLORS.map(c =>
    `<label class="color-opt" title="${c.name}">
      <input type="radio" name="gcal-def-color" value="${c.id}" ${c.id === current ? 'checked' : ''}>
      <span class="color-swatch" style="background:${c.hex}"></span>
    </label>`
  ).join('');
  grid.querySelectorAll('input[name="gcal-def-color"]').forEach(radio => {
    radio.addEventListener('change', () => GCal.setDefaultColor(radio.value));
  });
}
...
function renderSettings() {
  ...
  // render theme palette & gcal default color
  renderThemePalette();
  renderGCalDefaultColor();
}
